...
class EnvModel():
    ...
    def stateCompute(self, data, done_flag, done):
        ...
        if done_flag:
            self.model.add_node('goal', pos=(temp[0], temp[1]), value=100, confi=1)
        flag = False
        ...
        if not flag:
            ...
            if done:
                self.model.add_node(self.event.shape[0]-1, pos=(temp[0], temp[1]), value=-30, confi=1)
            else:
                self.model.add_node(self.event.shape[0]-1, pos=(temp[0], temp[1]), value=-1, confi=1)
    ...
    def chooseAction(self, state):
        if self.model.has_node('goal'):
            if nx.has_path(self.model, state, 'goal'):
                ...
                if np.random.uniform()<self.model.nodes[state]['confi']:
                    return action
                else:
    ...
    def exploreWihoutGoal(self):
        ...
        return action
        '''
        if self.exploreWithGoalFlag==0:
            return 0
        elif self.exploreWithGoalFlag==1:
            return 1
        elif self.exploreWithGoalFlag==2:
            return 2
        else:
            return 3
        '''
