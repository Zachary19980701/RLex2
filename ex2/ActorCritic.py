...
class Actor():
    ...
    def updateCriticModel(self, transModel, rewardModel, event):
        ...
        for i in range(1, self.event.shape[0]):
            ...
            if reward==-30:
                ...
                #print(tempReward)
                self.rewardModel.loc[i, 0] = tempReward
        '''
        for i in range(1, self.event.shape[0]):
            tempState = event[i]
            #print(tempState)
    ...
    def createActorModel(self):
        ...
        model.add(tf.keras.layers.Dense(self.actions, activation = 'softmax'))
    ...
    def rtUpdate(self, state, action, reward):
        ...
        self.actorModel.fit(stateVec, action, sample_weight=reward/30, verbose=0)
    ...
    def useModelUpdate(self, batchSzie=256):
        ...
        while(len(stateUpdateBuffer)<batchSzie):
            ...
            #print(nextState)
            if nextState!=0:
                reward = self.rewardModel.loc[nextState, 0]
        ...
        self.actorModel.fit(stateUpdateBuffer, actionUpdateBuffer, sample_weight=rewardUpdateBuffer/100, verbose=1)
        #print('total train')
        
        
    def saveModel(self):
        print('model saved')
        self.actorModel.save('/home/hzy/Desktop/ex2/dataset/actionModel.h5')
        self.StateModel.to_csv('/home/hzy/Desktop/ex2/dataset/StateModel.csv')
        self.rewardModel.to_csv('/home/hzy/Desktop/ex2/dataset/rewardModel.csv')
    ...
    def chooseAction(self, state):
        ...
        if self.checkStateExist(int(state)):
            ...
            if int(nextState)==0:
                ...
                self.epi = -self.rewardModel.loc[int(nextState), 0]/100
                #print(self.StateModel.loc[int(nextState), 0])
