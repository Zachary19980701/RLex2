...
class Env():
    ...
    def getReward(self):
        ...
        if self.goal.conlision(self.angent):
            ...
            if((((6.25-self.angent.x)**2 + (1.75-self.angent.y)**2 - (6.25-self.angent.last_x)**2 - (1.75-self.angent.last_y)**2))<0):
                reward = 0.1
                done = False
            else:
                reward = -0.1
    ...
    def doAction(self, action, distance=float(0.1)):
        ...
        #print(self.yaw)
        self.angent.last_x = self.angent.x
        self.angent.last_y = self.angent.y
